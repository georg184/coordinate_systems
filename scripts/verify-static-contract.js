...
assert.match(coreSource, /axisLengthScaleMin: 1,/);
assert.match(coreSource, /axisLengthScaleMax: 1\.6,/);
assert.equal((coreSource.match(/axisLengthScale,/g) || []).length, 2);
...
assert.match(appSource, /screenDirection\(axis\.direction, true, task\.axisLengthScale\)/);
...
assert.match(appSource, /if \(task\.showOrigin\) \{\s*svg\.appendChild\(createSvgElement\('circle', \{\s*class: 'origin-marker'/s);
assert.match(appSource, /class: 'origin-marker'[\s\S]*fill: AXIS_COLOR/);
const originMarkerSource = appSource.match(/class: 'origin-marker',[\s\S]*?\}\)\);/);
assert.ok(originMarkerSource, 'Missing isolated origin-marker renderer.');
assert.match(originMarkerSource[0], /r: 5,[\s\S]*fill: AXIS_COLOR/);
assert.doesNotMatch(originMarkerSource[0], /stroke/);
assert.match(appSource, /function originLabelPoint\(task, origin\)/);
assert.match(appSource, /task\.dimension === 2[\s\S]*origin\.x - 10[\s\S]*origin\.y \+ 12/);
assert.match(appSource, /perpendicular\.x \* 18[\s\S]*perpendicular\.y \* 18/);
assert.match(appSource, /originLabelPoint\(task, origin\),\s*'diagram-label-origin'/s);
